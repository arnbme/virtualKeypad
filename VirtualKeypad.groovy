...
metadata {
	...
	command "push", [[name:"btn", type:"STRING"]] 
...
def commandCustom(action,btn){
	if(checkInputCode(btn)){
		...
		if(action=="ReArm"){
			updateInputDisplay("Success.  Executing Disarm")
		...
		if(action=="ReArm"){
			updateInputDisplay("Success.  Executing Arm")
...
void updateInputDisplay(text){
	if(state.advancedButtonControl)
		sendEvent(name: "InputDisplay", value: text, displayed: false)
	else
		getChildDevice("${device.deviceNetworkId}-InputDisplay").updateInputDisplay(text)
}
...
def buttonPress(btn) {
    if (logEnable) log.debug "Entered push with event: ${btn}"
	if(btn == null){
		return
		}
